...
def draw_ellipses(img, nb_ellipses=20):
    ...
    for i in range(nb_ellipses):
        ...
        img = cv.ellipse(img, (x, y), (ax, ay), angle, 0, 360, col, -1)
...
def add_gausian_noise(img):
    ...
    add_gausian_noise = np.random.choice([True, False], p=[0.95, 0.05])
    if add_gausian_noise:
      # convert to torch tensor
      data: torch.tensor = K.image_to_tensor(img, keepdim=False)  # BxCxHxW
      # create the operator
      gauss = K.filters.GaussianBlur2d((11, 11), (100,100)) 
      # blur the image
      x_blur: torch.tensor = gauss(data.float())
      # convert back to numpy
      img_blur: np.ndarray = K.tensor_to_image(x_blur.byte())
    
      return img_blur
      
    return img

def pad_points(points, desired_num_points):
    pts_num=points.shape[0]
    one_padding = np.ones((pts_num,1), dtype=points.dtype)
    points = np.append(points, one_padding, axis=1)
    points = np.append(points, np.zeros((desired_num_points-pts_num,3)), axis=0)
    return points
